...
anal_opts.atom_laser.pulsedt=10.000e-3;
anal_opts.atom_laser.t0=1.088; %center i ntime of the first pulse
anal_opts.atom_laser.start_pulse=1; %atom laser pulse to start with
anal_opts.atom_laser.pulses=18;
...
while true
    ...
    if numel(anal_opts.tdc_import.shot_num)==0
        ...
        if sum(batch_data.mcp_tdc.all_ok)==0
            ...
            % undo the aliasing
            batch_data.osc_fit.trap_freq_recons(mask)=9*(1/anal_opts.atom_laser.pulsedt)+batch_data.osc_fit.model_coefs(mask,2,1);
