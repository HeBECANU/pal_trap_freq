...
tmp_xlim=[-40e-3, 40e-3];     %tight XY lims to eliminate hot spot from destroying pulse widths
tmp_ylim=[-40e-3, 40e-3];
tlim=[0,4];
anal_opts.tdc_import.txylim=[tlim;tmp_xlim;tmp_ylim];


anal_opts.atom_laser.pulsedt=10.000e-3;%8e-3;%
anal_opts.atom_laser.t0=0.415;%1.088; %center i ntime of the first pulse
anal_opts.atom_laser.start_pulse=2; %atom laser pulse to start with
anal_opts.atom_laser.pulses=100;%18;

anal_opts.atom_laser.appr_osc_freq_guess=[40,40,40];
...
anal_opts.osc_fit.dimesion=3; %Sel ect coordinate to bin. 1=X, 2=Y.
n_zone=3;%for tight trap n_zone = 10, for tune-out trap n_zone = 3

anal_opts.history.shots=10;
    ...
    if numel(anal_opts.tdc_import.shot_num)==0
        ...
        try
            ...
            if sum(batch_data.mcp_tdc.all_ok)==0
                ...
                % undo the aliasing
                batch_data.osc_fit.trap_freq_recons(mask)=n_zone*(1/anal_opts.atom_laser.pulsedt)+batch_data.osc_fit.model_coefs(mask,2,1);
